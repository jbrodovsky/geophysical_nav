...
def plot_estimate_error(
    ...
    axes[0].fill_between(
        trajectory["distance"] / 1000,
        np.maximum(
            result["estimate_error"] + result["planar_variance"] / 2, map_resolution
        ),
        drift_rate,
    ...
    axes[0].fill_between(
        ...
        result["estimate_error"] + result["planar_variance"] / 2,
        map_resolution,
        where=result["estimate_error"] + result["planar_variance"] / 2
        <= map_resolution,
        color="green",
        alpha=0.3,
...
def run_post_processing(output_dir: str, title_str: str, recovery_offset: float = 0.0):
            ...
            if file.endswith(".h5"):
                ...
                below_pixel = pd.concat(
                    [
                        below_pixel,
                        find_estimate_statistic(
                            result, result["estimate_error"] >= 452
                        ),
                    ]
                )
                ...
                certain_below_pixel = pd.concat(
                    [
                        certain_below_pixel,
                        find_estimate_statistic(
                            result, result["planar_variance"] >= 452
                        ),
                    ]
                )
...
def main():
    ...
    run_post_processing(
        os.path.join(output_dir, "localized"), "Particle Filter Estimate Error"
    )
