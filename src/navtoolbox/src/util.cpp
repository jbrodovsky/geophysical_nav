...
Eigen::MatrixXd util::mm_prod(const Eigen::MatrixXd& a, const Eigen::MatrixXd& b, bool at, bool bt) {
...
Eigen::MatrixXd util::mm_prod_symmetric(const Eigen::MatrixXd& a, const Eigen::MatrixXd& b) {
    return mm_prod(mm_prod(a, b), a, false, true);
}

// Matrix-vector product with optional transpose
Eigen::VectorXd util::mv_prod(const Eigen::MatrixXd& a, const Eigen::VectorXd& b, bool at) {
    Eigen::MatrixXd mat_a = at ? a.transpose() : a;
    return mat_a * b;
}

// Skew matrix creation from a 3D vector
Eigen::Matrix3d util::skew_matrix(const Eigen::Vector3d& vec) {
...
double util::compute_rms(const Eigen::VectorXd& data) {
    return std::sqrt((data.array().square().mean()));
}

// Reduce angle to the range [-180, 180] degrees
double util::to_180_range(double angle) {
...
Eigen::ArrayXd util::to_180_range(const Eigen::ArrayBase<Derived>& angles) {
